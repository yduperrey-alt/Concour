...
def _flux_google_news(requete: str) -> str:
    """Construit une URL de recherche Google Actualités (RSS) à partir d'une requête,
    avec encodage automatique — évite les erreurs d'encodage manuel des accents/espaces."""
...
FLUX_RSS = [
    ...
    # --- Google Actualités : mots-clés génériques de jeux-concours ---
    _flux_google_news('"jeux concours" gratuit'),
    _flux_google_news(
        '"grand jeu" OR "instants gagnants" OR "jeu 100% gagnant" '
        'OR "calendrier avent" OR "tirage au sort" OR "gagnez un" gratuit'
    ),

    # --- Google Actualités : marques qui organisent régulièrement des concours ---
    # (beaucoup de marques n'ont pas de flux RSS dédié aux concours ; on passe donc
    # par une recherche d'actualités groupée pour éviter de multiplier les requêtes)
    _flux_google_news('(LEGO OR Nintendo OR PlayStation OR Xbox OR Ubisoft) concours gratuit'),
    _flux_google_news('(Carrefour OR Auchan OR Fnac) concours gratuit'),
    _flux_google_news('(Coca-Cola OR Nutella OR Haribo OR Kinder OR Milka) concours gratuit'),
...
def recuperer_concours(categories_evitees=frozenset(), on_progress=None):
    ...
    for i, url in enumerate(FLUX_RSS, 1):
        if on_progress:
            on_progress(i, len(FLUX_RSS), url)
        try:
