...
# Icônes légères en caractères Unicode standard (aucune police/asset externe
# nécessaire). Si l'un de ces glyphes s'affiche en carré vide sur un appareil
# précis, remplace simplement la constante par une chaîne vide "" ou un mot.
ICONE_FAVORI_PLEIN = "♥"
ICONE_FAVORI_VIDE = "♡"
ICONE_ETOILE = "★"
ICONE_FERMER = "✕"
...
def recuperer_concours(categories_evitees=frozenset(), on_progress=None):
    resultats = []
    vus = set()
    ...
    # Téléchargement en parallèle : avec une centaine de flux, les enchaîner un
    # par un prendrait plusieurs minutes. On en télécharge plusieurs à la fois ;
    # le traitement (scoring, filtrage) reste lui séquentiel une fois chaque
    # flux reçu, donc pas besoin de verrous sur les listes partagées.
    with ThreadPoolExecutor(max_workers=MAX_FLUX_PARALLELES) as executor:
        futures = {executor.submit(_telecharger_flux, url): url for url in FLUX_RSS}
        ...
        for future in as_completed(futures):
            ...
            for entree in flux.entries:
                ...
                titre = nettoyer_titre_source(entree.get("title", "Sans titre"))
...
class ConcoursFinderApp(App):
    ...
    def build(self):
        ...
        root = BoxLayout(orientation="vertical", padding=(dp(18), dp(48), dp(18), dp(16)), spacing=dp(16))

        # --- En-tête façon "streaming" : gros titre + accroche + accès rapides ---
        entete = BoxLayout(orientation="vertical", size_hint=(1, None), height=dp(108), spacing=dp(6))

        ligne_titre = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(34), spacing=dp(10))
        accent_titre = Widget(size_hint=(None, None), size=(dp(5), dp(30)))
        with accent_titre.canvas:
            Color(*COULEUR_ACCENT)
            accent_rect = RoundedRectangle(radius=[dp(3)], pos=accent_titre.pos, size=accent_titre.size)
        ...
        ligne_actions = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(38), spacing=dp(8))
        for texte_btn, icone_btn, callback in (
            ("Favoris", ICONE_FAVORI_PLEIN, self._ouvrir_favoris),
            ("Historique", "", self._ouvrir_historique),
            ("Options", "", self._ouvrir_preferences),
        ):
            libelle = f"{icone_btn} {texte_btn}" if icone_btn else texte_btn
            btn = Button(text=libelle, font_size=dp(12), bold=True, color=COULEUR_TEXTE,
                         size_hint=(1, 1))
            stylise_bouton(btn, COULEUR_ONGLET_INACTIF, rayon=19)
        ...
        stylise_bouton(self.bouton_recherche, COULEUR_ACCENT, rayon=16)
        ...
        ligne_recherche = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(48), spacing=dp(8))
        ...
        bouton_effacer = Button(text=ICONE_FERMER, font_size=dp(14), bold=True, color=COULEUR_TEXTE,
                                 size_hint=(None, 1), width=dp(48))
        stylise_bouton(bouton_effacer, COULEUR_ONGLET_INACTIF, rayon=14)
        ...
        onglets = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(46), spacing=dp(10))
        self.boutons_pages = {}
        libelles_pages = {
            1: "Top lots",
            2: "Bons plans",
        ...
        for num_page, libelle in libelles_pages.items():
            btn = Button(text=libelle, font_size=dp(13), bold=True, color=COULEUR_TEXTE)
            stylise_bouton(btn, COULEUR_ONGLET_INACTIF, rayon=20)
        ...
        self.indicateur_pull = Label(
            text="", font_size=dp(12), color=COULEUR_ACCENT, bold=True,
        ...
        # Espacement généreux entre les cartes : c'est ce qui donne le plus
        # immédiatement une impression "haut de gamme" façon streaming.
        self.liste = GridLayout(cols=1, spacing=dp(14), size_hint_y=None, padding=(0, dp(6)))
    ...
    def _ajouter_ligne_concours(self, i, c):
        ...
        contenu = BoxLayout(orientation="vertical", spacing=dp(6), size_hint_y=None,
                             padding=(dp(14), dp(14), 0, dp(14)))

        ligne_badge = BoxLayout(size_hint_y=None, height=dp(24), spacing=dp(6))
        ...
        badge.width = badge.texture_size[0] + dp(20)
        badge.bind(size=lambda inst, val: setattr(inst, "text_size", val))
        with badge.canvas.before:
            Color(*couleur_palier)
            badge_rect = RoundedRectangle(radius=[dp(12)], pos=badge.pos, size=badge.size)
        ...
        score_lbl = Label(
            text=f"{c['score']} pts", font_size=dp(11), color=COULEUR_TEXTE_ATTENUE,
        ...
        def _update_hauteurs(instance, texture_size, ligne=ligne, contenu=contenu, item=item):
            ...
            contenu.height = texture_size[1] + dp(24) + dp(6) + dp(28)
        ...
        actions = BoxLayout(orientation="vertical", size_hint=(None, 1), width=dp(44), spacing=dp(8),
                             padding=(0, dp(14), 0, dp(14)))

        est_favori = self._est_favori(c["lien"])
        bouton_fav = Button(
            text=ICONE_FAVORI_PLEIN if est_favori else ICONE_FAVORI_VIDE,
            font_size=dp(17), bold=True, color=(1, 1, 1, 1),
            size_hint=(None, None), size=(dp(40), dp(40)),
        )
        stylise_bouton(bouton_fav, COULEUR_ACCENT if est_favori else COULEUR_ONGLET_INACTIF, rayon=20)
        ...
        case = CheckBox(size_hint=(None, None), size=(dp(32), dp(32)), color=COULEUR_TEXTE)
    ...
    def _afficher_details(self, c):
        ...
        contenu = BoxLayout(orientation="vertical", spacing=dp(12), padding=dp(18))

        libelle_palier, couleur_palier, icone_palier = infos_palier(c["score"])
        ligne_badge_popup = BoxLayout(orientation="horizontal", size_hint_y=None, height=dp(26), spacing=dp(8))
        texte_badge_popup = f"{icone_palier} {libelle_palier}" if icone_palier else libelle_palier
        badge_popup = Label(
            text=texte_badge_popup, font_size=dp(12), bold=True,
            color=(0.07, 0.07, 0.07, 1) if couleur_palier == COULEUR_PREMIUM else (1, 1, 1, 1),
            size_hint=(None, None), height=dp(26), halign="center", valign="middle",
        )
        badge_popup.texture_update()
        badge_popup.width = badge_popup.texture_size[0] + dp(22)
        badge_popup.bind(size=lambda inst, val: setattr(inst, "text_size", val))
        with badge_popup.canvas.before:
            Color(*couleur_palier)
            badge_popup_rect = RoundedRectangle(radius=[dp(13)], pos=badge_popup.pos, size=badge_popup.size)
        badge_popup.bind(pos=lambda inst, val: setattr(badge_popup_rect, "pos", inst.pos))
        badge_popup.bind(size=lambda inst, val: setattr(badge_popup_rect, "size", inst.size))
        ligne_badge_popup.add_widget(badge_popup)
        ligne_badge_popup.add_widget(Label(
            text=f"{c['score']} pts", font_size=dp(12), color=COULEUR_TEXTE_ATTENUE,
            size_hint=(None, 1), width=dp(50), halign="left", valign="middle",
        ))
        ligne_badge_popup.add_widget(BoxLayout())
        contenu.add_widget(ligne_badge_popup)

        label_date = Label(
            text=c["date_limite_texte"] if c.get("date_limite_texte") else "",
            font_size=dp(13), bold=True, color=COULEUR_URGENCE,
        ...
        boutons = BoxLayout(orientation="horizontal", size_hint_y=None, height=dp(52), spacing=dp(10))
        bouton_ouvrir = Button(text=f"Voir la page {ICONE_FLECHE}", bold=True, color=(1, 1, 1, 1))
        stylise_bouton(bouton_ouvrir, COULEUR_ACCENT, rayon=14)
        bouton_fermer = Button(text=ICONE_FERMER, bold=True, color=COULEUR_TEXTE, size_hint=(None, 1), width=dp(52))
        stylise_bouton(bouton_fermer, COULEUR_ONGLET_INACTIF, rayon=14)
        ...
        bouton_favori = Button(
            text=f"{ICONE_FAVORI_PLEIN} Retirer des favoris" if est_favori else f"{ICONE_FAVORI_VIDE} Ajouter aux favoris",
            bold=True, color=(1, 1, 1, 1), size_hint_y=None, height=dp(50),
        )
        ...
        def _on_press_favori(inst):
            nouvel_etat = self._basculer_favori(c)
            bouton_favori.text = (f"{ICONE_FAVORI_PLEIN} Retirer des favoris" if nouvel_etat
                                   else f"{ICONE_FAVORI_VIDE} Ajouter aux favoris")
        ...
        etat_popup = {"ouverte": True}
        popup.bind(on_dismiss=lambda inst: etat_popup.__setitem__("ouverte", False))
        ...
        # Si la popup est déjà fermée quand la réponse arrive, on ne touche à rien.
        threading.Thread(
            target=self._verifier_page_concours,
            args=(c, statut_verif, label_date, _ajouter_info, etat_popup),
            daemon=True,
        ).start()

    def _verifier_page_concours(self, c, statut_verif, label_date, ajouter_info, etat_popup):
        lien = c["lien"]
        ...
        if texte_page is None:
            self._maj_verification(statut_verif, label_date, None, None, echec=True, etat_popup=etat_popup)
            return
        ...
        self._maj_verification(statut_verif, label_date, nouvelles_infos, date_texte, echec=False,
                                ajouter_info=ajouter_info, etat_popup=etat_popup)

    @mainthread
    def _maj_verification(self, statut_verif, label_date, nouvelles_infos, date_texte, echec,
                           ajouter_info=None, etat_popup=None):
        if etat_popup is not None and not etat_popup["ouverte"]:
            return  # la popup a été fermée entretemps, inutile de toucher aux widgets
