...
FLUX_RSS = [
    ...
    # --- Presse jeux vidéo (annoncent souvent des concours/goodies) ---
    "https://blog.playstation.com/feed",
    "https://news.xbox.com/en-us/feed",
    "https://store.steampowered.com/feeds/news.xml",
    "https://feeds.ign.com/ign/all",
    "https://www.jeuxactu.com/rss/ja.rss",
    "https://www.gameblog.fr/rss.xml",
...
MAX_FLUX_PARALLELES = 10  # évite de solliciter Google Actualités trop agressivement d'un coup
...
class ConcoursFinderApp(App):
    ...
    def build(self):
        ...
        accent_titre = Widget(size_hint=(None, None), size=(dp(6), dp(28)))
        ...
        entete.add_widget(accent_titre)
        ...
        ligne_recherche = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(44), spacing=dp(8))
        ...
        bouton_effacer = Button(text="X", font_size=dp(14), bold=True, color=(1, 1, 1, 1),
        ...
        self.indicateur_pull = Label(
            text="", font_size=dp(12), color=COULEUR_TEXTE_ATTENUE, bold=True,
    ...
    def _ajouter_ligne_concours(self, i, c):
        ...
        # Alternance de couleur de fond (carte arrondie) + fine bordure pour la détacher du fond
        with ligne.canvas.before:
            ...
            Color(0.24, 0.24, 0.28, 1)
            bordure = Line(rounded_rectangle=(ligne.x, ligne.y, ligne.width, ligne.height, dp(12)), width=dp(1))

        def _sync_fond(inst, *_a):
            rect.pos = inst.pos
            rect.size = inst.size
            bordure.rounded_rectangle = (inst.x, inst.y, inst.width, inst.height, dp(12))
        ...
        with badge.canvas.before:
            Color(*couleur_score)
            badge_rect = RoundedRectangle(radius=[dp(9)], pos=badge.pos, size=badge.size)
        ...
        ligne_badge = BoxLayout(size_hint_y=None, height=dp(21))
        ...
        if date_obj:
            ...
            if 0 <= jours_restants <= 5:
                ligne_badge.add_widget(Widget(size_hint=(None, 1), width=dp(6)))
                ...
                with urgence.canvas.before:
                    ...
                    urgence_rect = RoundedRectangle(radius=[dp(9)], pos=urgence.pos, size=urgence.size)
        ...
        def _update_hauteurs(instance, texture_size, ligne=ligne, contenu=contenu, item=item):
            ...
            contenu.height = texture_size[1] + dp(21)
            ligne.height = texture_size[1] + dp(21) + dp(20)
        ...
        # --- Actions secondaires, regroupées à droite (favori, puis suppression) ---
        bouton_fav = Button(
            text="Fav." if self._est_favori(c["lien"]) else "+ Fav",
            font_size=dp(9), bold=True, color=(1, 1, 1, 1),
            size_hint=(None, 1), width=dp(40),
        )
        stylise_bouton(bouton_fav, COULEUR_ACCENT if self._est_favori(c["lien"]) else COULEUR_ONGLET_INACTIF, rayon=9)
        ...
        ligne.add_widget(Widget(size_hint=(None, 1), width=dp(4)))  # évite les clics accidentels

        case = CheckBox(size_hint=(None, 1), width=dp(32), color=COULEUR_TEXTE)
